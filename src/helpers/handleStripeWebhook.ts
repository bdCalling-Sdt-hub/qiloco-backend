                    ...
                    try {
                         ...
                         const notificationData = {
                              ...
                              message: `You have a new order!\nOrder Number: ${order?.orderNumber}\nProducts: ${order?.products
                                   .map((product: any) => `${product?.productName} (Quantity: ${product?.quantity})`)
                         ...
                         try {
                              ...
                              throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to send notification');
                         }
                         ...
                         await Promise.all(
                              order.products.map(async (product: any) => {
                                   try {
                                        const updatedProduct = await Product.findByIdAndUpdate(product.productId, { $inc: { quantity: -product.quantity } }, { new: true });
                                   } catch (productUpdateError) {
                                        throw new AppError(StatusCodes.BAD_REQUEST, 'Failed to update product inventory');
                                   }
                              }),
                         );
                    } catch (orderUpdateError) {
                         throw new AppError(StatusCodes.BAD_REQUEST, 'Failed to process order');
                    }
               }),
          );
     } catch (error) {
          throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, `Overall webhook processing error: ${error}`);
     }
};
