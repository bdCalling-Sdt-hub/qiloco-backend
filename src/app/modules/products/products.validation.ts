...
const productSchema = z.object({
     ...
     quality: z.enum(['high', 'medium']).refine((val) => ['high', 'medium'].includes(val), {
          message: "Quality must be either 'high' or 'medium'",
     }),
     quantity: z.number().positive('Quantity must be a positive number').min(1, 'Quantity is required'),
