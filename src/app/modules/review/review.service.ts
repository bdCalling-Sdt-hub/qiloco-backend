...
// Create a review
const createReviewToDB = async (payload: any, userId: any, productId: string) => {
...
// Update a review
// Update a review
const updateReviewInDB = async (payload: any, userId: any, productId: string) => {
     const { rating, comment } = payload;

     const product = await findProductById(productId);

     // Find the review to update
     const reviewIndex = product.reviews.findIndex((review) => review.userId.toString() === userId.toString());
...
const getProductReviewStats = async (productId: string) => {
     ...
     const totalReviews = product.reduce((sum: number, item: any) => sum + item.count, 0);

     // Calculate the average rating
     const averageRating = product.reduce((sum: number, item: any) => sum + item._id * item.count, 0) / totalReviews;
