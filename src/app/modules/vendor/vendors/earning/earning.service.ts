...
const earnings = async (query: Record<string, unknown>) => {
     ...
     query.fields = 'orderNumber,email,createdAt,totalPrice, paymentIntentId, paymentStatus';
...
const getEarningById = async (id: string) => {
     const order = await Order.findById(id).select('ordernumber email createdAt totalPrice');
     if (!order) {
